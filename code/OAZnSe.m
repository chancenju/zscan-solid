...
for i=ZnSe.wvl
    ...
    ZnSe.oadata{j}.data=importdata("ZnSe/"+num2str(i)+"oa.txt").data;
    ZnSe.cadata{j}.wavelength=i;
    ZnSe.cadata{j}.data=importdata("ZnSe/"+num2str(i)+"ca.txt").data;
end
...
%     plot(Z{j},T{j},'o')
    ...
    plot(Z{j},Tmodel,'k');
    hold off;
    saveas(gca,['.\figure\', num2str(ZnSe.oadata{j}.wavelength),'.pdf'])
end  
%% 拟合
for j=[19]
    T3{j}=T2{j}./T{j}
